select
    ...
    , @costoflete := produccion.ufn_ProdFleteCostoTotal(tblprod.idproducto, @idinsumoflete) as costoflete    
    ...
    , @costoprocconversion := produccion.ufn_prodprocconversioncostototal(@escala, @cabidatroquel, @idperiodo, tblvpconv.idvariacion, tblprod.largobobina, tblprod.colaminadoalargo) as costoprocconversion    
    , @costoproclitografia := produccion.ufn_prodproclitografiacostototal(@escala, tblprod.pasadaslitograficas, @idperiodo, tblvplito.idvariacion) / @cabidatroquel as costoproclitografia
    ...
    , @costoprocguillotinado := produccion.ufn_prodprocguillotinadocostototal(@escala, tblprod.idproducto, @idperiodo, tblvpguillo.idvariacion, @cabidatroquel) as costoprocguillotinado
    ...
    , @costodesperdiciocaja := (((tbltroq.fibra * tbltroq.contrafibra) * produccion.ufn_insumocostototunidad(tblprod.insumo_idinsumo_material)) - @costocartoncaja) as costodesperdiciocaja        
    , @porcedesperdiciocaja := (@costodesperdiciocaja / @costocartoncaja) as porcedesperdiciocaja
...
from produccion.producto as tblprod
	...
	left join produccion.maquinavariprod as tblvplito on tblprod.maquinavariprod_idvariacion_rutalitografia =  tblvplito.idvariacion
	left join produccion.maquina as tblmrlito on tblvplito.maquina_idmaquina = tblmrlito.idmaquina
